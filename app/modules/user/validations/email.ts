...
const NON_UNIQUE_ERROR_MESSAGE =
  UserRepository.NON_UNIQUE_ERROR_MESSAGE.replace(FIELD_VAR, 'email');
const EMAIL_LENGTH_ERROR_MESSAGE = UserRepository.LENGTH_ERROR_MESSAGE.replace(
  FIELD_VAR,
  'email',
...
const emailValidation = email
  ...
  .isLength({ min: 5 })
  .withMessage(EMAIL_LENGTH_ERROR_MESSAGE)
  .custom(checkEmailIsUnique)
